...
/**
 * Internal library of functions for module minispeak
 *
 * All the minispeak specific functions, needed to implement the module
...
class helper
{
    ...
    public static function move_item($minispeak, $moveitemid, $direction)
    {
        global $DB;
        ...
        if (!$items = $DB->get_records(constants::M_QTABLE, array('minispeak' => $minispeak->id), $sort)) {
    ...
    public static function get_new_itemorder($cm){
        //get itemorder
        $comprehensiontest = new \qtype_minispeak\comprehensiontest($cm);
    ...
    public static function duplicate_item($minispeak,$context, $itemid)
    {
        global $CFG, $USER, $DB;
        

        if (!$item = $DB->get_record(constants::M_QTABLE, array('minispeak' => $minispeak->id, 'id'=>$itemid))) {
