...
use qtype_minispeak\privacy\provider;
...
global $CFG;
require_once($CFG->dirroot . '/question/type/minispeak/classes/privacy/provider.php');
...
class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('qtype_minispeak');
        $actual = \qtype_minispeak\privacy\provider::get_metadata($collection);
    ...
    public function test_export_user_preferences($name, $value, $expected) {
        $this->resetAfterTest();
        $user = $this->getDataGenerator()->create_user();
        set_user_preference("qtype_minispeak_$name", $value, $user);
        provider::export_user_preferences($user->id);
        $writer = writer::with_context(\context_system::instance());
        $this->assertTrue($writer->has_any_data());
        $preferences = $writer->get_user_preferences('qtype_minispeak');
        foreach ($preferences as $key => $pref) {
            $preference = get_user_preferences("qtype_minispeak_{$key}", null, $user->id);
            if ($preference === null) {
                continue;
            }
    ...
    /**
     * Create an array of valid user preferences for the Minispeak question type.
     *
